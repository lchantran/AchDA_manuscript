%% processConditions
% given a list of alignments (alignmentCodeList) and aligment columns 
% (aligmentColumn - i.e. what column in the trialTable has the timing info)
% and conditions (conditionsList)
...
%% extract the signals and calcualate means, std, z-scores
for alignmentEntry=alignmentCodeList
    ...
    for cond=conditionsList
        ...
        if iscell(processed.signals) % for photometry
            ...
            for counter=1:length(processed.signals)
                if ~isempty(processed.signals{counter})
                    ...
                    if ~isempty(mReturn)
                        ...
                        if exist('subtractMeanForVar', 'var') && subtractMeanForVar==true
                            disp('   using old alignment')
                            meanOff=mean(processed.ph.(alignmentCode).(condCode).photometry_mean{counter}(1:params.ptsKeep_before));
                        else
                            meanOff=0;
                        end
                        processed.ph.(alignmentCode).(condCode).photometry_std{counter}=std(mReturn-meanOff, 1);
                        processed.ph.(alignmentCode).(condCode).photometry_var{counter}=var(mReturn-meanOff, 1);
            ...
            params.notes=statusUpdate(params.notes, ...
                ['Running ' num2str(length(finalEventIndices)) ' trials on spiking with condition ' condCode ' aligned to ' alignmentCode ]);

            [mReturn, returnError]= ... % convert to spike rate
                extractMatrix(processed.signals, finalEventIndices, params.ptsKeep_before, params.ptsKeep_after);

            params.notes=statusUpdate(params.notes, ...
                returnError);

            if ~isempty(mReturn)
                processed.ph.(alignmentCode).(condCode).ph_mean=params.finalSampleRate * squeeze(mean(mReturn, 1));
                processed.ph.(alignmentCode).(condCode).ph_std=params.finalSampleRate * squeeze(std(mReturn, 0, 1));
                processed.ph.(alignmentCode).(condCode).ph_var=params.finalSampleRate^2 * squeeze(var(mReturn, 0, 1));
                processed.ph.(alignmentCode).(condCode).ph_z=...
                    processed.ph.(alignmentCode).(condCode).ph_mean./ ...
                    processed.ph.(alignmentCode).(condCode).ph_std;
            end
        end
    end
end
