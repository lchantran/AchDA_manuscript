...
%saveFolder='/Volumes/BS Office/Dropbox (HMS)/2ABT_data_bernardo/new_analysis/faster';
%saveFolder='/Volumes/BS Office/Dropbox (HMS)/2ABT_data_bernardo/new_analysis/faster/Figure2';
photomFolder='/Volumes/Neurobio/MICROSCOPE/Lynne/2ABT/1_photom';

saveFolder='~/Dropbox (HMS)/2ABT_data_bernardo/new_analysis/faster/Figure2';
%photomFolder='/Users/lynnechantranupong/Dropbox (HMS)/Lynne_behavior_transfer5/photometry';

%photomFolder='/Users/bernardo/Downloads/glusnfr1';
saveFolderOutput='~/Dropbox (HMS)/2ABT_data_bernardo/new_analysis/output';
...
skipMiceList={};
% dateList={'10132021', '10182021', '11182021', '11222021'}
%mouseList={'test'}
...
for dateC=dateList
  ...
  %      dateFolder=sprintf('/Users/lynnechantranupong/Dropbox (HMS)/Lynne_behavior_transfer5/photometry/%s/', date);
        cd(dateFolder)
        disp(['Scanning ' dateFolder])
        D=dir('WT60*')' %[dir('WT58*')' dir('WT60*')' dir('WT61*')' dir('WT63*')' dir('WT64*')' dir('WT65*')'];
%        D=[dir('S1355*')' dir('S1356*')' dir('S1357*')' dir('S1358*')' dir('S1376*')' dir('S1399*')' dir('S1400*') dir('S1401*') dir('S1448*') dir('S1449*') dir('S1450*') dir('S1451*')]';     % this is where you can select mice.  Enter dir('WT63*'), for example
        dCounter=1;
        mouseList={};
        for dfCounter=1:length(D)
...
%                 folder1='/Users/bernardo/Downloads/glusnfr1/photometry'
% 
%                 folder2 = sprintf('/Volumes/Neurobio/MICROSCOPE/Lynne/2ABT/1_behavior/%s/%s', date, mouse); %behavior
%                 folder2='/Users/bernardo/Downloads/glusnfr1'
                
%                 folder1 = sprintf('/Users/lynnechantranupong/Dropbox (HMS)/Lynne_behavior_transfer5/photometry/%s/%s', date, mouse); %photometry
%                 folder2 = sprintf('/Users/lynnechantranupong/Dropbox (HMS)/Lynne_behavior_transfer5/behavior/%s/%s', date, mouse); %behavior
                folder1 = sprintf('/Volumes/Neurobio/MICROSCOPE/Lynne/2ABT/1_photom/%s/%s', date, mouse); %photometry
                folder2 = sprintf('/Volumes/Neurobio/MICROSCOPE/Lynne/2ABT/1_behavior/%s/%s', date, mouse); %behavior
                reloaded=0;
            end
            ...
            if ~exist(folder1, 'dir') || ~exist(folder2, 'dir')
                ...
                if exist(pokeFile, 'file') && exist(statsFile, 'file')
                    ...
                    if (~reloaded && ~onlyRerunExisting) || (reloaded && rePreprocessIfExists)             
                        ...
                        outputRawFile=fullfile(saveFolderOutput, ['raw_' mouse '_' date '.mat']);

                        if exist(outputRawFile, 'file')
                            % load from file
                            load(outputRawFile);
                            disp('loaded raw file')
                            params.samplesPerChannel=size(output,2);
                        else
                            ...
                            if saveOutputInMatlab
                                disp('*** saving output in matlab format')
                                save(fullfile(saveFolderOutput, ['raw_' mouse '_' date '.mat']), 'output');
                            end
                        end
                        ...
                        if length(output)>10000 % then continue.  Otherwise, what is this?
                            ...
                            params.downSample=36; % or use 36 for the fast
                            params.finalSampleFreq=params.rawSampleFreq/(params.downSample); %% Changed for 108
                            ...
                            params.ptsKeep_before=(108/params.downSample)*40;
                            params.ptsKeep_after=(108/params.downSample)*100;
                            params.spectralWindow=2*params.downSample; %2*9*12;
                            ...
                            for channel=channelsToAnalze
                                ...
                                try
                                    [filtTimes, filtSignal_raw, rawSignal, spectFreqs, spectAmpVals, returnError] = spectBS(...
                                        flRaw, ...
                                        params...
                                        );
                                ...
                                if ~isempty(filtSignal)
                                    ...
                                    dSignal_raw=mean(...
                                        reshape(filtSignal_raw(1:(blockLength*nBlocks)), blockLength, nBlocks),...
                                        1);                                    
                                    ...
                                    if firstRun
                                        ...
                                        processed.signals_raw=cell(1, params.numChannels);
                                    ...
                                    processed.signals_raw{channel}=dSignal_raw;
                    ...
                    if params.finalSamples>0
                        ...
                        if ~behaviorErrorFlag
                            ...
                            pGraphSummary(processed.ph.SI, {'Rew', 'NoRew'}, [ 1 5 6], tRange=[-121 0.018])
                        %    clear processed
                        %    clear params
                        %    clear output
                        else
