...
nEventTypes=10;     % how many stimuli or behavioral events in the model    
...
gaussStretch=2;
b = 2*rand*normpdf((-4:4)/gaussStretch);%+rand*(exp(-(0:8)/gaussStretch));
b=b/max(b);
...
linkProbability=0.4; % what is the probability that a neuron "hears" an event type
...
for neuronIndex=testNeuron
    ...
    legend
    title(['reconstruction ' num2str(floor(100*mean(1./(yy.^0.5))))]);
end
